...
export default function MyWorkExperience() {
  ...
  return (
    <section className="flex flex-col items-center w-full max-w-[1298px]">
      <h2 className="flex justify-center items-center w-full h-58 text-[35px] sm:text-[50px] md:text-[64px] font-[600] text-[#344054]">
        My{" "}
      ...
      <main className="w-full flex flex-col">
        {expList.map((item, index) => (
          <section className="w-full flex flex-col lg:flex-row lg:justify-between px-10 md:px-15 pb-12 relative text-[20px] md:text-[30px] lg:text-[40px] ">
            <article className="flex flex-col items-start w-full lg:w-[40%] relative">
              <h3 className="text-[#344054] font-[600]">{item.title}</h3>
              <p className="text-[#98A2B3] text-[12px] md:text-[18px] lg:text-[24px]">
                {item.subtitle}
              </p>
            </article>
            <div className="h-full flex justify-center absolute lg:static md:left-2 left-0 ">
              ...
              <div
                ...
                }  before:w-9 before:h-9 before:rounded-[50%] bg-white z-1 lg:scale-100 md:scale-50 ms:scale-30 scale-30 `}
              ></div>
            </div>
